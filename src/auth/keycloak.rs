...
pub fn keycloak_login(oauth2: OAuth2<KeycloakUserInfo>, cookies: &CookieJar<'_>) -> Redirect {
    oauth2.get_redirect(cookies, &["openid", "profile", "email"]).unwrap()
}

#[get("/auth/keycloak")]
pub async fn keycloak_callback(token: TokenResponse<KeycloakUserInfo>, cookies: &CookieJar<'_>) -> Result<Redirect, Debug<Error>> {
...
#[get("/auth/logout/keycloak")]
pub fn keycloak_logout(oauth2: OAuth2<KeycloakUserInfo>, cookies: &CookieJar<'_>) -> Redirect {
...
#[get("/auth/register/keycloak")]
pub fn keycloak_register(oauth2: OAuth2<KeycloakUserInfo>, cookies: &CookieJar<'_>) -> Redirect {
